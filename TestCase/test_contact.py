...
import logging
import os,sys
sys.path.append(os.path.dirname(__file__) + os.sep + '../')  # 找到上级目录的路径添加加到path
...
class Testcontactcase(unittest.TestCase):
    ...
    def test_contact(self, name, email, star, phone, note, assertion):
        try:
            ...
            logging.info("测试用例:%s, 期望: \'%s\',, 失败" %
                         ((name, email, star, phone, note), assertion))
        except Exception as e:
            logging.error("未知错误, 错误信息:" + str(traceback.format_exc()))
        else:
            logging.info("测试用例:%s, 期望: \'%s\', 通过" %
                         ((name, email, star, phone, note), assertion))
